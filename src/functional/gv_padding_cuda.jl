#=
Padding-Operations for the GPU
=#
...
"""
function zero_pad_nd(input::CuArray{T, N}, padding::NTuple{N, Int}) where {T <: Number, N}
    ...
    output = CUDA.zeros(eltype(input), output_shape)

    # actual computation
    ranges = [dim_padding + 1:dim_size - dim_padding for (dim_size, dim_padding) in zip(output_shape, padding)]
    output[ranges...] = input
...
"""
function zero_pad_2d(input::CuArray{T, 4}, padding::NTuple{2, Int}) where {T <: Number}
