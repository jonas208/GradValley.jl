...
function test_layer_initializations()
    ...
    batch_norm = BatchNorm2d(12, epsilon=1e-05, momentum=0.1, affine=true, track_running_stats=true, activation_function=nothing)
    ...
    flatten = Reshape((300, )) # features = C*H*W = 12*5*5 = 300
    fully_connected_1 = Fc(300, 150, activation_function=nothing, init_mode="kaiming_uniform") # in_features = C*H*W = 12*5*5 = 300
    ...
    submodel_convs = SequentialContainer([conv, depthwise_conv])
    ...
    model = SequentialContainer([submodel_feature_extractor, submodel_fcs])
...
function test_data_loader_initialization()
    ...
    data_loader = DataLoader(get_function, 100, batch_size=32, shuffle=true, drop_last=true) # or dataset_size=1000 (or similar, but not divisible by batch_size for drop_last)
...
function simulate_training(model, data_loader, optim_1, optim_2, optim_3)
    for (batch, (images_batch, targets_batch)) in enumerate(data_loader)
        ...
        testmode!(model)
        predictions_batch_test_mode = forward(model, images_batch)
        trainmode!(model)
        predicitions_batch = forward(model, images_batch)
