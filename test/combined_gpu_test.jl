...
function simulate_training(model, data_loader, optim_1, optim_2, optim_3)
    for (batch, (images_batch, targets_batch)) in enumerate(data_loader)
        ...
        images_batch, targets_batch = CuArray.((images_batch, targets_batch))
        # test forward pass (only affects batchnorm: in training mode and in test mode)
        testmode!(model)
        predictions_batch_test_mode = forward(model, images_batch, CUDA.rand(dtype, 20, size(images_batch)[end]), 5)
        trainmode!(model)
        # predicitions_batch = forward(model, images_batch, CUDA.rand(dtype, 4), 5)
        predicitions_batch = forward(model, images_batch, CUDA.rand(dtype, 20, size(images_batch)[end]), 5)
